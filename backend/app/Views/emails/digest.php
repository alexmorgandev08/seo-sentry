...
<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.5;color:#1d2327;max-width:640px;">
    ...
    <?php if ($impaired) : ?>
        <div style="border-left:4px solid #bd8600;background:#fcf9e8;padding:12px 16px;margin-bottom:20px;">
            <strong><?php esc_html_e('Monitoring is impaired', 'seo-sentry'); ?></strong><br>
            <?php echo esc_html($impaired['reason']); ?><br>
            <span style="color:#646970;font-size:13px;">
                <?php
                printf(
                    /* translators: %s: date */
                    esc_html__('Since %s. Until this is fixed, no all-clear can be trusted.', 'seo-sentry'),
    ...
    <?php if (!empty($findings)) : ?>
        <p><?php esc_html_e('These changes were detected on your site:', 'seo-sentry'); ?></p>
        ...
        <?php foreach ($findings as $finding) : ?>
            ...
            <div style="border:1px solid #dcdcde;border-left:4px solid <?php echo esc_attr($colour); ?>;padding:14px 16px;margin-bottom:16px;">
                ...
                <div style="font-weight:600;margin:2px 0 6px;">
                    <?php echo esc_html($label ?: __('Site-wide', 'seo-sentry')); ?>
                </div>

                <?php if ($explanation) : ?>
                    <p style="margin:0 0 8px;"><?php echo esc_html($explanation['what']); ?></p>
                    <p style="margin:0 0 8px;color:#50575e;"><strong><?php esc_html_e('Why it matters:', 'seo-sentry'); ?></strong> <?php echo esc_html($explanation['why']); ?></p>
                    <p style="margin:0 0 8px;color:#50575e;"><strong><?php esc_html_e('What to check:', 'seo-sentry'); ?></strong> <?php echo esc_html($explanation['check']); ?></p>
                <?php endif; ?>

                <table style="font-size:13px;border-collapse:collapse;margin-top:6px;">
                    <tr>
                        <td style="color:#646970;padding:2px 12px 2px 0;vertical-align:top;"><?php esc_html_e('Before', 'seo-sentry'); ?></td>
                    ...
                    <tr>
                        <td style="color:#646970;padding:2px 12px 2px 0;vertical-align:top;"><?php esc_html_e('After', 'seo-sentry'); ?></td>
                ...
                <?php if (!empty($events)) : ?>
                    <p style="margin:10px 0 0;font-size:13px;color:#646970;">
                        <?php esc_html_e('Site events shortly before this change (they may be related, but we cannot be certain):', 'seo-sentry'); ?><br>
    ...
    <?php if (!empty($resolved)) : ?>
        <div style="border:1px solid #dcdcde;border-left:4px solid #008a20;padding:14px 16px;margin-bottom:16px;">
            <div style="font-weight:600;margin-bottom:6px;"><?php esc_html_e('All clear', 'seo-sentry'); ?></div>
            <p style="margin:0 0 8px;"><?php esc_html_e('These critical problems are no longer present:', 'seo-sentry'); ?></p>
            <ul style="margin:0;padding-left:18px;">
                <?php foreach ($resolved as $finding) : ?>
                    <li>
                        <?php
                        $label       = $pageLabels[$finding->target_id] ?? __('Site-wide', 'seo-sentry');
                        $explanation = $explanations[$finding->id] ?? null;
                        echo esc_html($label);
                        if ($explanation) {
